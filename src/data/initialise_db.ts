...
export function intialiseDB() {
  ...
  dynamicDb.exec(`
    CREATE TABLE IF NOT EXISTS club_events (
      ...
      event_timestamp INTEGER GENERATED ALWAYS AS (json_extract(event_data, '$.timestamp')) VIRTUAL,
      event_date TEXT GENERATED ALWAYS AS (json_extract(event_data, '$.event_date')) VIRTUAL
    );
    CREATE INDEX IF NOT EXISTS idx_events_dedupe ON club_events(club_id, event_name, event_timestamp);
    CREATE INDEX IF NOT EXISTS idx_events_date ON club_events(event_date);
  ...
  try {
      dynamicDb.exec(`ALTER TABLE club_creation_requests ADD COLUMN admin_secret TEXT`);
